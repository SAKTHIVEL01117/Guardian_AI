...
class FatigueTracker:
    ...
    def process_frame_landmarks(self, img_bgr: np.ndarray, face_data):
        now = time.time()
        self.timestamps.append(now)

        # 1. Extract Landmark points for EAR and MAR
        ear = 0.28
        mar = 0.15
        pitch = 0.0
        yaw = 0.0
        ...
        # 2. Eye Closure & Blink Counter Logic
        ...
        if ear < CLOSED_EAR_THRESHOLD:
            ...
            if self.is_eye_closed:
                # Transited from closed to open -> recorded blink
        ...
        # 3. Yawn Counter Logic
        YAWN_MAR_THRESHOLD = 0.52
        ...
        # 4. Compute PERCLOS (Percentage of time eye is closed)
        ...
        shoulder_posture = "Aligned" if abs(roll) < 10.0 else "Tilted Shoulders"
